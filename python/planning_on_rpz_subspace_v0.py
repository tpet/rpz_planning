...
def load_data(LABEL_NUMBER):
    ...
    net.load_state_dict(torch.load("../dem_predictor/data/s2d_network/net_weights_s2d", map_location='cpu'))
...
def interp_yaw(yaw_in):
    ...
    if 0 < idx < 8:
        yaw_idx = [idx - 1, idx]
        #w = (yaw_bins[idx] - yaw_deg) / 45
    if idx == 8:
        yaw_idx = [idx - 1, 0]
        #w = (yaw_bins[idx] - yaw_deg) / 45
    w = (np.radians(yaw_bins[idx]) - yaw_in) / (np.pi/4)
    return yaw_idx, w


def visualize(x, pcl, robot, output_dem, LOSS, az = 0):
    plt.figure(2)
    plt.clf()
    ax = plt.axes(projection='3d')
    Z = output_dem
    ...
    # ax.bar3d(XX.ravel()-GRID_RES/2, YY.ravel()-GRID_RES/2, Z.ravel(), GRID_RES, GRID_RES, -0.01, shade=True, alpha=0.2, zsort='max')
    # ax.bar3d(heightmap_grid[0].ravel()-patch_size/2, heightmap_grid[1].ravel()-patch_size/2, Z.ravel(), patch_size, patch_size, -0.01, shade=True, alpha=0.5)  # color=cm.jet(norm(lamb.detach().numpy()).ravel()), shade=True, alpha=0.5)
    #ax.quiver(XX.ravel(), YY.ravel(), Z.ravel(), normals[0, :, :].detach().numpy().ravel(), normals[1, :, :].detach().numpy().ravel(), normals[2, :, :].detach().numpy().ravel(), length=0.1, color='g')
    plt.title('LOSS = ' + str('{0:.2f}'.format(LOSS.squeeze().detach().numpy())) + ',  drpz_at_point = ' + str('{0:.2f}'.format(LOSS.squeeze().detach().numpy())))
    ...
    ax.view_init(elev=50, azim=-az)  # elev=15, azim=visu)

    plt.pause(0.001)
    plt.draw()

def rpz_diff(t, x0, y0, sig2):
    ...
    for k in range(TRAJECTORY_LENGTH):
        if k==0:
            ...
            yaw_idx, w = interp_yaw(yaw[k])

            C = x[k].long()
            R = y[k].long()
            kernel = torch.exp(-0.5 * ((torch.tensor(yy) + x[k].detach() - x[k]).square() + (torch.tensor(xx) + y[k].detach() - y[k]).square()) / sig2)
            kernel = kernel/kernel.sum()

            roll[k] = (kernel * (roll_all[yaw_idx[0], (R - sz):(R + sz + 1), (C - sz):(C + sz + 1)] * w + roll_all[yaw_idx[1], (R - sz):(R + sz + 1), (C - sz):(C + sz + 1)] * (1 - w))).sum()
            pitch[k] = (kernel * (pitch_all[yaw_idx[0], (R - sz):(R + sz + 1), (C - sz):(C + sz + 1)] * w + pitch_all[yaw_idx[1], (R - sz):(R + sz + 1), (C - sz):(C + sz + 1)] * (1 - w))).sum()
            z[k] = (kernel * (z_all[yaw_idx[0], (R - sz):(R + sz + 1), (C - sz):(C + sz + 1)] * w + z_all[yaw_idx[1], (R - sz):(R + sz + 1), (C - sz):(C + sz + 1)] * (1 - w))).sum()
...
def project_to_camera(roll, pitch, yaw, x, y, z, pcl):
    Rx = np.zeros((3, 3))
    Ry = np.zeros((3, 3))
    Rz = np.zeros((3, 3))
    t = np.zeros((3, 1))
...
t = 10*torch.ones([TRAJECTORY_LENGTH-1, 2])  # [x1-x0, y1-y0, ....]
t[:,1] = 0.01*t[:,1]
x0 = torch.tensor(60)
y0 = torch.tensor(100)

##### Define pointcloud to be covered #####
pcl = torch.rand(100,3)
pcl[:,0] = 5*pcl[:,0] +10
pcl[:,1] = 5*pcl[:,1] +10
...
kernel_sz = 2*sz + 1
sig = kernel_sz/5
sx = np.linspace(-(kernel_sz - 1) / 2., (kernel_sz - 1) / 2., kernel_sz)
xx, yy = np.meshgrid(sx, sx)
sig2 = torch.tensor(sig).square()
sig2.requires_grad_(True)
...
for epoch in range(EPOCHS):
    ...
    roll, pitch, yaw, x, y, z, ROLL, PITCH, Z = rpz_diff(t, x0, y0, sig2)
    print("rpz_diff time:", time.time() - time_start, "sec")

    # define camera
    #pxl = project_to_camera(roll[k], pitch[k], yaw[k], x[k], y[k], z[k], pcl)
    #define LOSS as a function of covered voxels

    LOSS = (z.square().sum() + pitch.square().sum() + roll.square().sum() )+ K_v * (t[:,0] - t0[:,0]).square().sum() + K_yaw * (t[:,1] - t0[:,1]).square().sum()

    visualize(torch.cat((ROLL, PITCH, yaw, x, y, Z), dim=1), pcl, robot, output_dem, LOSS, epoch)

    # estimate gradient
    time_start = time.time()
    LOSS.backward()
    print("LOSS.backward() time:", time.time() - time_start, "sec")
    print('loss:', LOSS.detach().numpy() ) #, 'tx=',x[0,3].detach().numpy(), 'ty=',x[0,4].detach().numpy()) #,'diff=', diff[sz,sz].detach().numpy())
